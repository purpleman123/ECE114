...
int inputIntInRange(int lower, int upper){ // function created to scan for number guess input and then ensure it is a valid input
    int input;
       while ( 0==scanf("%ld", &input) ) // first loop ensures input is a number and not a character or word
        ...
        while ( (input > upper || input < lower) ) // then it makes sure the 
        {
            printf("Try again. Enter an integer between %d and %d: ", lower, upper);
            scanf("%d", &input);
            fflush(stdin);
